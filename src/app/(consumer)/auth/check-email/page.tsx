...
import { ArrowLeft, AlertCircle, CheckCircle2, Loader2, MailCheck, RefreshCcw, X } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="auth-panel relative z-10 flex flex-col justify-center border-r border-white/10 bg-black/30 px-6 shadow-[0_0_50px_rgba(0,0,0,0.3)] backdrop-blur-2xl supports-backdrop-filter:bg-black/20 lg:px-12">
        <Button asChild data-auth-back aria-label="Back to login" variant="ghost" size="icon" className="absolute z-20 h-8 w-8 rounded-full bg-white/10 text-white transition-colors hover:bg-white/20">
          <Link href={emailVerified ? continueHref : appendNextPath("/auth/login", nextPath)}>
        ...
        <div className="mx-auto w-full max-w-90">
          <div className="mb-8 space-y-4 text-center lg:text-left">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs font-medium leading-relaxed text-zinc-300 md:text-sm">
                {emailVerified ? (
                  <>
                    <span className="font-bold text-white">{email}</span> is already confirmed for this account.
                  </>
                ) : isDeliveryFailed ? (
                  <>
                    Your account was created, but we could not send the verification email. Try sending it again.
                  </>
                ) : (
                  <>
                    Open the email we sent to {email ? <span className="font-bold text-white">{email}</span> : "your email"} to continue.
                  </>
                )}
              </p>
            </div>
          ...
          <div className="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4 text-xs font-medium text-zinc-300 backdrop-blur-md">
            {emailVerified ? (
              <p>No further email action is needed. Continue to your previous task.</p>
            ) : (
              <>
