  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex min-h-screen flex-col justify-center border-r border-white/10 bg-black/30 px-6 shadow-[0_0_50px_rgba(0,0,0,0.3)] backdrop-blur-2xl supports-backdrop-filter:bg-black/20 lg:px-12">
        <Link href={appendNextPath("/seller/login", nextPath)}>
          <Button aria-label="Back to seller login" variant="ghost" size="icon" className="absolute left-4 top-4 z-20 h-8 w-8 rounded-full bg-white/10 text-white transition-colors hover:bg-white/20">
          ...
          <div className="mb-8 space-y-4 text-center lg:text-left">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs font-medium leading-relaxed text-zinc-300 md:text-sm">
                We created your seller account. Open the email we sent to{" "}
      ...
      <div className="relative z-10 hidden flex-col justify-end p-16 lg:flex xl:p-24">
        <Link href="/sell">
          <Button aria-label="Back to sell page" variant="ghost" size="icon" className="absolute right-6 top-6 rounded-full border border-white/10 bg-black/20 text-white backdrop-blur-md transition-colors hover:bg-black/40">
            <X className="h-5 w-5" />
          </Button>
        </Link>
      </div>
...
function SellerCheckEmailFallback() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#101010] p-6 text-white">
