...
/**
 * @file DisputeQueueTabs.tsx
 * @module features/admin-disputes/components
 * @description
 * Tactile queue navigation tabs for the Returns, Claims & Disputes Control Center.
 * Features live queue count badges, smooth cubic-bezier transitions, and warm surface styling.
 */
...
import {
  DISPUTE_QUEUE_TABS,
  type DisputeQueueTabKey,
} from "../types";

interface DisputeQueueTabsProps {
  activeTab: DisputeQueueTabKey;
  onTabChange: (tab: DisputeQueueTabKey) => void;
  tabCounts?: Record<DisputeQueueTabKey, number | null>;
  className?: string;
}
...
> = {
  ...
  in_review: Search,
  waiting_evidence: Clock,
  escalated: AlertTriangle,
  resolved: CheckCircle2,
  all: Inbox,
};

export function DisputeQueueTabs({
  activeTab,
  onTabChange,
  tabCounts,
  className,
}: DisputeQueueTabsProps) {
  return (
    <div
      role="tablist"
      aria-label="Dispute queue lifecycle navigation"
      className={cn(
      ...
      {DISPUTE_QUEUE_TABS.map((tab) => {
        ...
        return (
          <button
            ...
            aria-controls={`dispute-panel-${tab.key}`}
